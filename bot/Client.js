...
import giveawaysManager from "./utilities/giveaway.js";
import logger from "./utilities/logger.js";
import Database from "./handlers/db.js";
import extractor from "@discord-player/extractor";
import * as _ from "lodash";
import * as DBL from "@top-gg/sdk";
import express from "express";
const { Lyrics } = extractor;
        ...
        this.functions = {
            ...
            createOptionsEmbed: (command, message) => {
                const optionsEmbed = new Discord.MessageEmbed()
                    ...
                    .addField("Permissions", command.userPermissions?.length ? command.userPermissions.map((x) => `\`${x}\``).join(", ") : "No user permissions required.")
            ...
            try {
                for (let file of commands) {
                    if (props.name) {
                        ...
                        table.addRow(file, props.name, dir, props.aliases ? props.aliases.join(", ") : "None.", "Not Loaded -> Missing a help.name, or help.name is not a string.");
                        continue;
                    }
                    props.category = dir;
...
function recursiveShallowClone(obj1, obj2) {
    for (const key of Object.getOwnPropertyNames(obj2)) {
        if (_.isObject(obj2[key]) && obj1[key])
            recursiveShallowClone(obj1[key] ?? obj1, obj2[key]);
        else
...
function loadStructures() {
    ...
    for (const file of structures) {
        const name = file.split(".")[0], struct = Discord[name];
    }
}
