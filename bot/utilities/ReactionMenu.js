...
export const Menu = class extends EventEmitter {
    /**
     * Creates a menu.
    ...
    constructor(channel, author, pages, ms = 180000) {
        ...
        if (this.channel.type !== "dm") {
            ...
            if (missingPerms.length)
                this.channel.send(`Looks like you haven't given the client ${missingPerms.join(", ")} permissions in #${this.channel}. This perm is needed for basic menu operation. You'll probably experience problems using menus in this channel.`);
        }
        else {
    ...
    start() {
        ...
        this.channel
            ...
            if (this.channel.type === "dm") {
                ...
                this.author
                    .send(`${error.toString()} (whilst trying to send menu message) | You're probably missing 'SEND_MESSAGES' or 'EMBED_LINKS' in #${this.channel.name} (${this.channel.guild.name}), needed for sending the menu message.`)
                    .catch(() => { });
            }
        });
    }
        ...
        for (const reaction in this.currentPage.reactions) {
            this.menu.react(reaction).catch((error) => {
                if (error.toString().indexOf("Unknown Emoji") >= 0) {
                    throw new Error(`${error.toString()} (whilst trying to add reactions to message) | The emoji you were trying to add to page "${this.currentPage.name}" (${reaction}) probably doesn't exist. You probably entered the  wrong when adding a custom emoji.`);
                }
                else {
