...
const fetchSuggestionChannel = async (guildId) => {
    ...
    if (!guildSchema)
        throw new Error("Invalid guild ID provided for getting suggestion channel.");
    if (!guildSchema.suggestionChannel || guildSchema.suggestionChannel === "null")
        return null;
    return await client.channels.fetch(guildSchema.suggestionChannel).catch(() => { });
};
export { fetchSuggestionChannel };
